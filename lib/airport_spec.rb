...
describe Airport do
  ...
  describe "#land(plane)" do
    ...
    it "raises an error when there is no room to land" do
      ...
      tplane = double(:tplane)
      allow(tplane).to receive(:landed?).and_return(false)
      airport = Airport.new(weather)
      plane = Plane.new
      airport.capacity.times { airport.land(plane) }
      expect { airport.land(tplane) }.to raise_error "No room to land"
    end
  ...
  describe "#takeoff(plane)" do
    it "allows a plane to take off and confirm that its not at airport" do
      ...
      tplane = double(:tplane)
      allow(tplane).to receive(:landed?).and_return(false, true)
      airport = Airport.new(weather)
      airport.land(tplane)
